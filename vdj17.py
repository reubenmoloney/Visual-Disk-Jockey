...
def display_window(objects,background_color_object):
    ...
    while running:
        ...
        if(volume >= 0.7):
            tempBackgroundColorObj = background_color_object.value
            tempBackgroundColorObj.nextColor()
            background_color_object.value = tempBackgroundColorObj
        
        unCalcBackgroundColor = background_color_object.value.getColor()
        backgroundColor = [int(unCalcBackgroundColor[0]*volume), int(unCalcBackgroundColor[1]*volume), int(unCalcBackgroundColor[2]*volume)]
...
def display_ui(objects, background_color_object):
    pygame.init()
    pygame.display.set_icon(programIcon)
    ...
    show_add_bg_color_form = False
    show_set_bg_color_form = False
    ...
    add_triangles_effect_button = pygame_gui.elements.UIButton(
        ...
        text="AddTriagnlesEffect",
        manager=manager
    )

    # Create the "Set background Color" button
    set_background_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((50, 150), (120, 50)),
        text="Set BG-Color",
        manager=manager
    )

    # Create the "Add Background Color" button
    add_background_color_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((200, 150), (120, 50)),
        text="Add BG-Color",
        manager=manager
    )

    # Create the "Toggle Background cycle" button
    toggle_background_cycle_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((350, 150), (120, 50)),
        text="Toggle BG-color Cycle",
        manager=manager
    )
    ...
    def add_background_color_form():
        """Create the form for adding a color to the cycle of background colors."""
        form_elements.clear()

        labels = ["colorR", "colorG", "colorB"]
        y_offset = 120
        x_offset = 600
    ...
    def set_background_color_form():
        """Create the form for setting the background temporarily as a color."""
        form_elements.clear()
            ...
            if event.type == pygame.USEREVENT:
                if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                    if event.ui_element == add_star_button:
                        ...
                        show_set_bg_color_form = False
                        save_as_preset_form()
                    elif event.ui_element == add_background_color_button:
                        clear_form()
                        show_sine_wave_form = False
                        show_star_form = False
                        show_circle_form = False
                        ...
                        show_save_preset_form = False
                        show_triangles_effect_form = False
                        show_add_bg_color_form = True
                        show_set_bg_color_form = False
                        add_background_color_form()
                    elif event.ui_element == set_background_button:
                        clear_form()
                        show_sine_wave_form = False
                        show_star_form = False
                        show_circle_form = False
                        ...
                        show_triangles_effect_form = False
                        show_add_bg_color_form = False
                        show_set_bg_color_form = True
                        set_background_color_form()
                    elif (show_star_form or show_circle_form or show_sine_wave_form or show_preset_form or show_save_preset_form or show_triangles_effect_form or show_add_bg_color_form or show_set_bg_color_form) and event.ui_element == form_elements["submit"]:
                        ...
                        elif(show_set_bg_color_form):
                            newBackgroundObject = background_color_object.value
                            newBackgroundObject.setColor([int(data["colorR"]),int(data["colorG"]),int(data["colorB"])])
                            background_color_object.value = newBackgroundObject
                        elif(show_add_bg_color_form):
                            newBackgroundObject = background_color_object.value
                            newBackgroundObject.addColor([int(data["colorR"]),int(data["colorG"]),int(data["colorB"])])
                            background_color_object.value = newBackgroundObject
                        show_star_form = False
                        show_circle_form = False
                        show_sine_wave_form = False
                        ...
                        for loop, button in enumerate(preset_buttons):
                            if event.ui_element == button:
                                ...
                                for obj in newObjects:
                                    objects.append(obj)
                                break
                    elif(event.ui_element == toggle_background_cycle_button):
                        newBackgroundObject = background_color_object.value
                        newBackgroundObject.toggleCycle()
                        background_color_object.value = newBackgroundObject
...
def start_display_ui(objects,background_color_object):
    ...
    process = multiprocessing.Process(target=display_ui, args=(objects,background_color_object))
    process.start()
    return process
...
if __name__ == "__main__":
    with multiprocessing.Manager() as manager:
        ...
        background_color_object.value = BackgroundColor([[0,0,0]], True)
            ...
            while display_window_process.is_alive():
                ...
                if(keyboard.is_pressed('c')):
                    if not display_ui_process.is_alive():
                        print("Restarting control panel...")
